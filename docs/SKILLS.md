...
| `membrane scan [path]` | Offline scan. `--json` machine output; `--report md\|html [--out file]` = Generative-AI Technical-Debt Report (rule/dir/file aggregates + A–F grade + masked source line per finding); `--fail-on=blocking\|warning\|never` drives the exit code. |
